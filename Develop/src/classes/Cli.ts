  ...
  findVehicleToTow(truck: Truck): void {
    inquirer
      ...
      .then((answers) => {
        ...
        const select = answers.vehicleToTow;
        if (select.vin === truck.vin) {
          console.log(`The ${select.make} ${select.model} cannot tow itself!`);
          this.performActions();
        } else {
          console.log(select.vin);
          truck.tow(select);
          this.performActions();
          ...
          for (let i = 0; i < this.vehicles.length; i++) {
            if (this.vehicles[i].vin === this.selectedVehicleVin) {
              if (this.vehicles[i] instanceof Motorbike) {
                ...
                console.log(`${this.vehicles[i].make} ${this.vehicles[i].model} is not a motorbike.`);
              }
            }
          }
        }
